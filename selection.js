const Selection = (function() {
  ...
  function _selection() {
    ...
    const dictionaryConfig = {
      enurl: 'https://www.dictionary.com/browse/', //English
      esurl: 'https://www.spanishdict.com/translate/', //Spanish
      frurl: 'http://www.french-linguistics.co. /dictionary/', //French add .html after text
      zhurl: 'https://www.mdbg.net/chinese/dictionary?page=worddict&wdrst=0&wdqb=', //Chinese
      hiurl: 'https://hi.oxforddictionaries.com/परिभाषा/' , //Hindi
    ...
    function dictionaryButton() {
      const dictionarybtn = new Button(dictionaryConfig.icon, function() {
        text = validateDictValue(text);
        url = ""
        //Point to appropriate dictionary based on Browser Language
        if (getBrowserLanguage() == "en"){
          url = dictionaryConfig.enurl;
        }
        else if (getBrowserLanguage() == "es"){
          url = dictionaryConfig.esurl;
        }
        else if (getBrowserLanguage() == "fr"){
          url = dictionaryConfig.frurl + '.html';
        }
        else if (getBrowserLanguage() == "zh"){
          url = dictionaryConfig.zhurl;
        }
        else if (getBrowserLanguage() == "hi"){
          url = dictionaryConfig.hiurl;
        }
        else if (getBrowserLanguage() == "ar"){
          url = dictionaryConfig.arurl;
        }
        else if (getBrowserLanguage() == "ms"){
          url = dictionaryConfig.msurl;
        }
        else if (getBrowserLanguage() == "ru"){
          url = dictionaryConfig.ruurl;
        }
        else if (getBrowserLanguage() == "bn"){
          url = dictionaryConfig.bnurl;
        }
        else if (getBrowserLanguage() == "pt"){
          url = dictionaryConfig.pturl;
        }
        else{
          url = dictionaryConfig.enurl;
        }
        popupwindow(url + encodeURIComponent(text), 'Dictionary', 900, 540);
        return false;
      });
    ...
    function validateDictValue(text){
      ...
      if (values.length > 1){
        //If more than one word grab the first word
    ...
    function appendIcons() {
      ...
      {feature:'copy',call:copyButton()},{feature:'speak',call:speakButton()},{feature:'dictionary',call:dictionaryButton()}]
