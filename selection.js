  ...
  function _selection() {
    ...
    const menuOptions = {
      twitter: {
        ...
        onclick: function() {
          popupwindow(this.url + encodeURIComponent(text) + ' ' + windowUrl, 'Share', 550, 295);
          return false;
        }
      },
      facebook: {
      ...
      translate: {
        ...
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" enable-background="new 0 0 24 24" width="24" height="24" class="selection__icon">'+
          ...
          '</svg>',
        onclick: function() {
          popupwindow(this.url + getBrowserLanguage() + '/' + text, 'Translate', 900, 540);
          return false;
        }
      },
      speak: {
    ...
    function appendIcons() {
      ...
      Object.keys(menuOptions).forEach((option) => {
        ...
        if (menu[option]) {
          const btn = bindOptionsToNewButton(icon, menuOptions[option]);
          div.appendChild(btn);
          count++;
        }
      })
